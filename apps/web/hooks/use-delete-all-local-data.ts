...
import { useGuidedUsaJobsStore } from '@/store/guidedUsaJobsStore';
import {
  PROFILE_STORAGE_KEY,
 ...
 *
 * DAY 44 UPDATE:
 * Added GUIDED_USAJOBS_GOALS_STORAGE_KEY for Guided USAJOBS goals.
 */
const STORAGE_KEYS = [
  PROFILE_STORAGE_KEY,
  JOB_SEARCH_STORAGE_KEY,
 ...
 *
 * DAY 44 UPDATE:
 * Added resetGuidedUsaJobs to clear Guided USAJOBS goals and session state.
 */
export function useDeleteAllLocalData() {
  ...
  /**
   * Day 44: Reset Guided USAJOBS store.
   */
  const resetGuidedUsaJobs = useGuidedUsaJobsStore(function (state) {
  ...
  const deleteAllLocalData = async function () {
    ...
    resetGuidedUsaJobs(); // Day 44: Clears Guided USAJOBS state
