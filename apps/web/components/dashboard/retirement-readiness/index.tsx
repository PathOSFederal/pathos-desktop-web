...
export function RetirementReadinessCard() {
  ...
  const { visible, isSensitiveHidden } = useDashboardCardVisibility('dashboard.retirementReadiness');

  const title = user.currentEmployee ? 'Retirement Readiness' : 'Career Timeline';
    ...
    <Card className="border-border bg-card">
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          ...
          <DashboardCardVisibilityToggle cardKey="dashboard.retirementReadiness" />
        </div>
      </CardHeader>
      <CardContent>
        {visible ? (
          ...
          <CardHiddenPlaceholder title={title} cardKey="dashboard.retirementReadiness" />
        )}
      </CardContent>
    </Card>
  );
