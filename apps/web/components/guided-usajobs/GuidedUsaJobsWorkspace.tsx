...
export function GuidedUsaJobsWorkspace() {
  ...
  const embedStatus = useGuidedUsaJobsStore(function (state) {
    return state.embedStatus;
  });
  ...
  const setEmbedStatus = useGuidedUsaJobsStore(function (state) {
    return state.setEmbedStatus;
  });
  ...
  // ---------------------------------------------------------------------------
  // Local UI state for selection dragging
  // ---------------------------------------------------------------------------
  ...
  useEffect(function () {
    ...
    const timeoutId = window.setTimeout(function () {
      const latestStatus = useGuidedUsaJobsStore.getState().embedStatus;
  ...
  // ---------------------------------------------------------------------------
  const getRelativePoint = function (event: React.MouseEvent) {
  ...
  const clampValue = function (value: number, min: number, max: number) {
    if (value < min) {
      return min;
    }
    if (value > max) {
  ...
  const finalizeRegionSelection = async function (region: GuidedUsaJobsRegion) {
    ...
    setResponse(null);
    setScreenshot(null);
    applyEvent('SELECT_REGION', 'Region selected from embedded surface.');

    const overlay = overlayRef.current;
    const captureResult = await captureGuidedUsaJobsScreenshot(region, overlay);
    ...
    applyEvent('CAPTURE_SCREENSHOT', 'Local pixel capture completed.');
  ...
  const handleMouseDown = function (event: React.MouseEvent) {
    if (!askModeEnabled || interactionState !== 'ARMED') {
      return;
    }
  ...
  const handleMouseUp = function (event: React.MouseEvent) {
    if (!askModeEnabled || interactionState !== 'ARMED') {
      return;
    }
    ...
    const dragWidth = Math.abs(point.x - start.x);
    const dragHeight = Math.abs(point.y - start.y);
    const isClick = dragWidth < 12 && dragHeight < 12;

    let region: GuidedUsaJobsRegion;
    if (isClick) {
      const defaultWidth = 180;
      const defaultHeight = 120;
      const rawX = point.x - defaultWidth / 2;
      const rawY = point.y - defaultHeight / 2;

      const clampedX = clampValue(rawX, 0, rect.width - defaultWidth);
      const clampedY = clampValue(rawY, 0, rect.height - defaultHeight);
      ...
      region = {
        x: clampValue(x, 0, rect.width),
        y: clampValue(y, 0, rect.height),
        width: clampValue(width, 24, rect.width),
        height: clampValue(height, 24, rect.height),
      };
    }
